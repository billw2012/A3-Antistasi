	...
	case "heal": {
				if (player != _flag) then
					...
					if ([_flag] call A3A_fnc_fatalWound) then
						{
						_accion = _flag addAction [format ["<t color='#0020FF'>Revive %1</t> <img size='1.8' <img image='\a3\ui_f\data\IGUI\Cfg\holdactions\holdAction_reviveMedic_ca.paa' />",name _flag], "Revive\actionRevive.sqf",nil,6,true,true,"","!(_this getVariable [""ayudando"",false]) and (isNull attachedTo _target)",4];
						_flag setUserActionText [_accion,format ["<t color='#0020FF'>Revive %1</t>",name _flag],"<t size='2'><img image='\a3\ui_f\data\IGUI\Cfg\holdactions\holdAction_reviveMedic_ca.paa'/></t>"];
						}
					else
						{
						_accion = _flag addAction [format ["<t color='#2020FF'>Revive %1</t> <img size='1.8' <img image='\a3\ui_f\data\IGUI\Cfg\holdactions\holdAction_revive_ca.paa' />",name _flag], "Revive\actionRevive.sqf",nil,6,true,true,"","!(_this getVariable [""ayudando"",false]) and (isNull attachedTo _target)",4];
						_flag setUserActionText [_accion,format ["<t color='#2020FF'>Revive %1</t>",name _flag],"<t size='2'><img image='\a3\ui_f\data\IGUI\Cfg\holdactions\holdAction_revive_ca.paa'/></t>"];
						};
					};
				};
	case "heal1":
		...
		if (player != _flag) then
			...
			if ([_flag] call A3A_fnc_fatalWound) then
				{
				_accion = _flag addAction [format ["<t color='#0020FF'>Revive %1</t> <img size='1.8' <img image='\a3\ui_f\data\IGUI\Cfg\holdactions\holdAction_reviveMedic_ca.paa' />",name _flag], "Revive\actionRevive.sqf",nil,6,true,false,"","!(_this getVariable [""ayudando"",false]) and (isNull attachedTo _target)",4];

				_flag setUserActionText [_accion,format ["<t color='#0020FF'>Revive %1</t>",name _flag],"<t size='2'><img image='\a3\ui_f\data\IGUI\Cfg\holdactions\holdAction_reviveMedic_ca.paa'/></t>"];
				}
			else
				{
				_accion = _flag addAction [format ["<t color='#2020FF'>Revive %1</t> <img size='1.8' <img image='\a3\ui_f\data\IGUI\Cfg\holdactions\holdAction_revive_ca.paa' />",name _flag], "Revive\actionRevive.sqf",nil,6,true,false,"","!(_this getVariable [""ayudando"",false]) and (isNull attachedTo _target)",4];
				_flag setUserActionText [_accion,format ["<t color='#0020FF'>Revive %1</t>",name _flag],"<t size='2'><img image='\a3\ui_f\data\IGUI\Cfg\holdactions\holdAction_revive_ca.paa'/></t>"];
				};
			...
			_accion = _flag addAction [format ["<t color='#2000FF'>Carry %1</t> <img image='\A3\ui_f\data\igui\cfg\actions\take_ca.paa' size='1.6' shadow=2 />",name _flag], "Revive\carry.sqf",nil,5,true,false,"","(isPlayer _this) and (_this == _this getVariable ['owner',objNull]) and (isNull attachedTo _target) and !(_this getVariable [""ayudando"",false]);",4];
			_flag setUserActionText [_accion,format ["<t color='#2000FF'>Carry %1</t>",name _flag],"<t size='2'><img image='\A3\ui_f\data\igui\cfg\actions\take_ca.paa'/></t>"];
